...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin && selectedRole === "admin" && adminCode !== "112233") {
      toast({ title: "Error", description: "Invalid admin access code.", variant: "destructive" });
      return;
    }
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-all ${selectedRole === "admin" ? "border-secondary bg-secondary/10" : "border-border bg-muted hover:border-muted-foreground"}`}
                    >
                      <Shield className={`w-6 h-6 ${selectedRole === "admin" ? "text-secondary" : "text-muted-foreground"}`} />
                      <span className={`text-sm font-display font-semibold ${selectedRole === "admin" ? "text-secondary" : "text-muted-foreground"}`}>ADMIN</span>
                    </button>
                  </div>
                  ...
                  <AnimatePresence>
                    {selectedRole === "admin" && (
                      ...
                      >
                        <Shield className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary z-10" />
                        <Input
                          type="text"
                          ...
                          required
                          className="pl-10 bg-muted border-border focus:border-secondary transition-colors"
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
