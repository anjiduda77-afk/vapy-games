...
import {
  ...
  Home, X, Crown, MonitorPlay, MessageSquare
} from "lucide-react";
...
const DashboardSidebar = ({ isOpen, onClose }: DashboardSidebarProps) => {
  ...
  const navItems = [
    ...
    { icon: MessageSquare, label: "Chat Room", path: "/dashboard/chat" },
    ...(role === "admin"
          ...
          >
            ...
            <div className="px-6 pb-6 border-b border-sidebar-border">
              <div className="flex items-center gap-4">
                {/* Avatar */}
                <div className="w-14 h-14 rounded-full border-2 border-primary neon-border overflow-hidden bg-muted flex items-center justify-center flex-shrink-0">
                  {profile?.avatar_url ? (
                    <img
              ...
              {role && (
                <span className={`inline-flex items-center gap-1 mt-3 px-3 py-1 rounded-full text-xs font-display font-semibold ${role === "admin"
                    ? "bg-secondary/20 text-secondary"
                    : "bg-primary/20 text-primary"
                  }`}>
                  {role === "admin" ? <Shield className="w-3 h-3" /> : <Gamepad2 className="w-3 h-3" />}
                  {role.toUpperCase()}
                </span>
              )}
            </div>
              ...
              {navItems.map(item => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all font-body text-base font-medium ${isActive
